    ...
    <div class="w-full max-w-md bg-white rounded-lg shadow-xl p-8">
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 @error('name') border-red-500 @enderror" value="{{ old('name') }}" required autocomplete="name" autofocus>
                @error('name')
            ...
            <div class="mb-6">
                ...
                <input type="password" name="password_confirmation" id="password_confirmation" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200" required autocomplete="new-password">
            </div>

            <div class="mb-6">
                <label for="role" class="block text-gray-700 text-sm font-bold mb-2">Register As</label>
                <select name="role" id="role" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 @error('role') border-red-500 @enderror" required>
                    <option value="user" {{ old('role') == 'user' ? 'selected' : '' }}>Patient</option>
                    {{-- Staff registration should ideally be handled by an admin, but including for completeness based on backend route --}}
        ...
        <p class="text-center text-gray-600 text-sm mt-8">
            Already have an account? <a href="{{ route('login') }}" class="font-bold text-blue-600 hover:text-blue-800">Login here</a>.
        </p>
    </div>
</div>
@endsection
