...
<div class="container mx-auto px-4 py-8">
    ...
    <h1 class="text-4xl font-bold text-gray-800 mb-8">Edit Payment: #{{ $payment->id }}</h1>
        ...
        <form action="{{ route('payments.update', $payment->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" id="amount" class="shadow
                        ...
                        @error('amount') border-red-500 @enderror" value="{{ old('amount') ?? $payment->amount }}" required min="0.01">
                    @error('amount')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <select name="payment_method" id="payment_method" class="shadow appearance-none border
                        ...
                        <option value="Online Transfer" {{ (old('payment_method') ?? $payment->payment_method) == 'Online Transfer' ? 'selected' : '' }}>Online Transfer</option>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="payment_date" id="payment_date" class="shadow appearance-none border
                    ...
                    border-red-500 @enderror" value="{{ old('payment_date') ?? $payment->payment_date->format('Y-m-d') }}" required>
                @error('payment_date')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" id="notes" rows="4" class="shadow appearance-none border
                    ...
                    red-500 @enderror" placeholder="Any additional notes about the payment...">{{ old('notes') ?? $payment->notes
                    }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
