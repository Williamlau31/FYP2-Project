        ...
        <form action="{{ route('appointments.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="staff_id" id="staff_id" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 @error('staff_id') border-red-500 @enderror" required>
                    ...
                    @foreach($staff as $s)
                        <option value="{{ $s->id }}" {{ old('staff_id') == $s->id ? 'selected' : '' }}>{{ $s->name }} ({{ $s->specialization ?? $s->role }})</option>
                    @endforeach
                </select>
                @error('staff_id')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="date" name="date" id="date" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 @error('date') border-red-500 @enderror" value="{{ old('date') }}" required min="{{ \Carbon\Carbon::today()->format('Y-m-d') }}">
                    @error('date')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="time" name="time" id="time" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 @error('time') border-red-500 @enderror" value="{{ old('time') }}" required>
                    @error('time')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="appointment_type" id="appointment_type" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 @error('appointment_type') border-red-500 @enderror" value="{{ old('appointment_type') }}" placeholder="e.g., General Checkup, Follow-up">
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" id="notes" rows="4" class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200 @error('notes') border-red-500 @enderror" placeholder="Any specific notes for the appointment...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
