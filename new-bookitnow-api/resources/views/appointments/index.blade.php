...
<div class="container mx-auto px-4 py-8">
    ...
    @if(Auth::user()->isAdmin())
        <div class="flex justify-between items-center mb-6">
            <a href="{{ route('appointments.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 flex items-center">
            ...
            <form action="{{ route('appointments.index') }}" method="GET" class="flex items-center space-x-4">
                <input type="date" name="date_from" value="{{ request('date_from') }}" class="form-input rounded-lg border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                <span class="text-gray-600">to</span>
                <input type="date" name="date_to" value="{{ request('date_to') }}" class="form-input rounded-lg border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                <select name="status" class="form-select rounded-lg border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                ...
                <a href="{{ route('appointments.index') }}" class="bg-gray-400 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300">
                    Reset
                </a>
            </form>
        </div>
                                    ...
                                    <div class="flex item-center justify-center">
                                        ...
                                        @if(Auth::user()->isAdmin())
                                            ...
                                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                                <button @click="open = !open" class="w-4 transform hover:text-purple-500 hover:scale-110" title="Update Status">
                                                    <i class="fas fa-sync-alt"></i>
                                                </button>
                                                <div x-show="open" @click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                                                    <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                                        <form action="{{ route('appointments.update-status', $appointment->id) }}" method="POST" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">
                                                            ...
                                                            <button type="submit" class="block w-full text-left">Scheduled</button>
                                                        </form>
                                                        <form action="{{ route('appointments.update-status', $appointment->id) }}" method="POST" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">
                                                            @csrf
                                                            @method('PATCH')
